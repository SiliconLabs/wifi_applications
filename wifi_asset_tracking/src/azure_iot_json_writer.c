                                         ...
                                         uint32_t ulBufferSize)
{
  ...
  if ((pxWriter == NULL) || (pucBuffer == NULL) || (ulBufferSize == 0)) {
    ...
    xJSONSpan =
      az_span_create(( uint8_t * ) pucBuffer, ( int32_t ) ulBufferSize);

    if (az_result_failed(xCoreResult =
                           az_json_writer_init(&pxWriter->_internal.xCoreWriter,
                                               xJSONSpan, NULL))) {
      AZLogError(("Could not initialize the JSON reader: core error=0x%08x",
                  xCoreResult));
  ...
  int32_t lValue)
{
  ...
  if ((pxWriter == NULL) || (pucPropertyName == NULL)
      || (ulPropertyNameLength == 0)) {
    AZLogError((
                 "AzureIoTJSONWriter_AppendPropertyWithInt32Value failed: invalid argument"));
    xResult = eAzureIoTErrorInvalidArgument;
  } else {
    xPropertyNameSpan = az_span_create(( uint8_t * ) pucPropertyName,
                                       ( int32_t ) ulPropertyNameLength);

    if (az_result_failed(xCoreResult =
                           az_json_writer_append_property_name(&pxWriter->
                                                               _internal.
                                                               xCoreWriter,
                                                               xPropertyNameSpan))
        || az_result_failed(xCoreResult =
                              az_json_writer_append_int32(&pxWriter->_internal.
                                                          xCoreWriter,
                                                          lValue))) {
      AZLogError(("Could not append property and int32: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendPropertyWithDoubleValue(
  AzureIoTJSONWriter_t *pxWriter,
  const uint8_t *pucPropertyName,
  uint32_t ulPropertyNameLength,
  double xValue,
  uint16_t usFractionalDigits)
{
  ...
  if ((pxWriter == NULL) || (pucPropertyName == NULL)
      || (ulPropertyNameLength == 0)) {
    AZLogError((
                 "AzureIoTJSONWriter_AppendPropertyWithDoubleValue failed: invalid argument"));
    xResult = eAzureIoTErrorInvalidArgument;
  } else {
    xPropertyNameSpan = az_span_create(( uint8_t * ) pucPropertyName,
                                       ( int32_t ) ulPropertyNameLength);
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_property_name(&pxWriter->
                                                               ...
                                                               xPropertyNameSpan))
        || az_result_failed(xCoreResult =
                              az_json_writer_append_double(&pxWriter->_internal.
                                                           xCoreWriter, xValue,
                                                           ( int32_t )
                                                           usFractionalDigits)))
    {
      AZLogError(("Could not append property and double: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
  ...
  bool xValue)
{
  ...
  if ((pxWriter == NULL) || (pucPropertyName == NULL)
      || (ulPropertyNameLength == 0)) {
    AZLogError((
                 "AzureIoTJSONWriter_AppendPropertyWithBoolValue failed: invalid argument"));
    xResult = eAzureIoTErrorInvalidArgument;
  } else {
    xPropertyNameSpan = az_span_create(( uint8_t * ) pucPropertyName,
                                       ( int32_t ) ulPropertyNameLength);
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_property_name(&pxWriter->
                                                               ...
                                                               xPropertyNameSpan))
        || az_result_failed(xCoreResult =
                              az_json_writer_append_bool(&pxWriter->_internal.
                                                         xCoreWriter,
                                                         xValue))) {
      AZLogError(("Could not append property and bool: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendPropertyWithStringValue(
  AzureIoTJSONWriter_t *pxWriter,
  const uint8_t *pucPropertyName,
  uint32_t ulPropertyNameLength,
  ...
  uint32_t ulValueLen)
{
  ...
  if ((pxWriter == NULL) || (pucPropertyName == NULL)
    ...
    AZLogError((
                 "AzureIoTJSONWriter_AppendPropertyWithStringValue failed: invalid argument"));
    xResult = eAzureIoTErrorInvalidArgument;
  } else {
    xPropertyNameSpan = az_span_create(( uint8_t * ) pucPropertyName,
                                       ( int32_t ) ulPropertyNameLength);
    xValueSpan = az_span_create(( uint8_t * ) pucValue,
                                ( int32_t ) ulValueLen);
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_property_name(&pxWriter->
                                                               ...
                                                               xPropertyNameSpan))
        || az_result_failed(xCoreResult =
                              az_json_writer_append_string(&pxWriter->_internal.
                                                           xCoreWriter,
                                                           xValueSpan))) {
      AZLogError(("Could not append property and string: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
int32_t AzureIoTJSONWriter_GetBytesUsed(AzureIoTJSONWriter_t *pxWriter)
{
  ...
  return az_span_size(az_json_writer_get_bytes_used_in_destination(&pxWriter->
                                                                   _internal.
                                                                   xCoreWriter));
}
...
AzureIoTResult_t AzureIoTJSONWriter_AppendJSONText(
  AzureIoTJSONWriter_t *pxWriter,
  ...
  uint32_t ulJSONLen)
{
  ...
  if ((pxWriter == NULL) || (pucJSON == NULL) || (ulJSONLen == 0)) {
    ...
    xJSONSpan = az_span_create(( uint8_t * ) pucJSON,
                               ( int32_t ) ulJSONLen);

    if (az_result_failed(xCoreResult =
                           az_json_writer_append_json_text(&pxWriter->_internal.
                                                           xCoreWriter,
                                                           xJSONSpan))) {
      AZLogError(("Could not append JSON text: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendPropertyName(
  AzureIoTJSONWriter_t *pxWriter,
  ...
  uint32_t ulValueLen)
{
  ...
  if ((pxWriter == NULL) || (pusValue == NULL) || (ulValueLen == 0)) {
    AZLogError(
      ("AzureIoTJSONWriter_AppendPropertyName failed: invalid argument"));
    xResult = eAzureIoTErrorInvalidArgument;
  } else {
    xPropertyNameSpan = az_span_create(( uint8_t * ) pusValue,
                                       ( int32_t ) ulValueLen);
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_property_name(&pxWriter->
                                                               ...
                                                               xPropertyNameSpan)))
    {
      AZLogError(("Could not append property name: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendNull(AzureIoTJSONWriter_t *pxWriter)
{
  ...
  if (pxWriter == NULL) {
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_null(&pxWriter->_internal.
                                                      xCoreWriter))) {
...
AzureIoTResult_t AzureIoTJSONWriter_AppendBeginObject(
  AzureIoTJSONWriter_t *pxWriter)
{
  AzureIoTResult_t xResult;
  az_result xCoreResult;
  ...
  if (pxWriter == NULL) {
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_begin_object(&pxWriter->
                                                              _internal.
                                                              xCoreWriter))) {
      AZLogError(("Could not append begin object: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendBeginArray(
  AzureIoTJSONWriter_t *pxWriter)
{
  AzureIoTResult_t xResult;
  az_result xCoreResult;
  ...
  if (pxWriter == NULL) {
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_begin_array(&pxWriter->
                                                             _internal.
                                                             xCoreWriter))) {
      AZLogError(("Could not append begin array: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendEndObject(
  AzureIoTJSONWriter_t *pxWriter)
{
  AzureIoTResult_t xResult;
  az_result xCoreResult;
  ...
  if (pxWriter == NULL) {
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_end_object(&pxWriter->_internal
                                                            .xCoreWriter))) {
      AZLogError(("Could not append end object: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
...
AzureIoTResult_t AzureIoTJSONWriter_AppendEndArray(
  AzureIoTJSONWriter_t *pxWriter)
{
  AzureIoTResult_t xResult;
  az_result xCoreResult;
  ...
  if (pxWriter == NULL) {
    ...
    if (az_result_failed(xCoreResult =
                           az_json_writer_append_end_array(&pxWriter->_internal.
                                                           xCoreWriter))) {
      AZLogError(("Could not append end array: core error=0x%08x",
                  xCoreResult));
      xResult = AzureIoT_TranslateCoreError(xCoreResult);
    } else {
      xResult = eAzureIoTSuccess;
