...
_get_remaining_span(az_json_writer *ref_json_writer, int32_t required_size)
{
  ...
  if ((az_span_size(remaining) < required_size)
    ...
    if (az_result_failed(ref_json_writer->_internal.allocator_callback(&context,
                                                                       &
                                                                       remaining)))
    {
  ...
  az_json_writer const *json_writer)
{
  ...
  if (_az_json_stack_peek(&json_writer->_internal.bit_stack)) {
    ...
    if (kind != AZ_JSON_TOKEN_PROPERTY_NAME) {
      ...
      _az_PRECONDITION(
        kind != AZ_JSON_TOKEN_NONE && kind != AZ_JSON_TOKEN_BEGIN_ARRAY);
  ...
  uint8_t byte)
{
  ...
  _az_json_stack_item stack_item = _az_json_stack_peek(
    &json_writer->_internal.bit_stack);
    ...
    default:
    {
      ...
      if (next_byte < _az_ASCII_SPACE_CHARACTER) {
        ...
        uint8_t array[_az_MAX_EXPANSION_FACTOR_WHILE_ESCAPING] = {
          ...
          '0',
          '0',
          _az_number_to_upper_hex(
            (uint8_t)(next_byte / _az_NUMBER_OF_HEX_VALUES)),
          _az_number_to_upper_hex(
            (uint8_t)(next_byte % _az_NUMBER_OF_HEX_VALUES)),
        };
        *remaining_destination = az_span_copy(*remaining_destination,
                                              AZ_SPAN_FROM_BUFFER(array));
  ...
  if (az_span_size(value) < az_span_size(*remaining_json)) {
    *remaining_json = az_span_copy(*remaining_json, value);
    ...
    while (az_span_size(value) != 0)
    {
      ...
      ref_json_writer->_internal.bytes_written += az_span_size(
        value_slice_that_fits);

      value = az_span_slice_to_end(value, az_span_size(value_slice_that_fits));
      *remaining_json = _get_remaining_span(ref_json_writer,
                                            _az_MINIMUM_STRING_CHUNK_SIZE);
      _az_RETURN_IF_NOT_ENOUGH_SIZE(*remaining_json,
                                    _az_MINIMUM_STRING_CHUNK_SIZE);
    }
  }
  return AZ_OK;
...
az_json_writer_append_string_small(az_json_writer *ref_json_writer,
                                   az_span value)
{
  _az_PRECONDITION(az_span_size(
                     value) <= _az_MAX_UNESCAPED_STRING_SIZE_PER_CHUNK);
  ...
  required_size += _az_json_writer_escaped_length(value,
                                                  &index_of_first_escaped_char,
                                                  false);
  ...
  if (index_of_first_escaped_char == -1) {
    ...
    // encode and copy.
    remaining_json
      = az_span_copy(remaining_json,
                     az_span_slice(value, 0, index_of_first_escaped_char));
...
static AZ_NODISCARD az_result
az_json_writer_append_string_chunked(az_json_writer *ref_json_writer,
                                     az_span value)
{
  _az_PRECONDITION(az_span_size(value)
                   > _az_MAX_UNESCAPED_STRING_SIZE_PER_CHUNK);

  az_span remaining_json = _get_remaining_span(ref_json_writer,
                                               _az_MINIMUM_STRING_CHUNK_SIZE);
  ...
  do
  {
    ...
    int32_t index_of_first_escaped_char = -1;
    _az_json_writer_escaped_length(value_slice,
                                   &index_of_first_escaped_char,
                                   true);
    ...
    if (index_of_first_escaped_char == -1) {
      _az_RETURN_IF_FAILED(
        az_json_writer_span_copy_chunked(ref_json_writer, &remaining_json,
                                         value_slice));
      ...
      int32_t written = _az_json_writer_escape_next_byte_and_copy(
        &remaining_json,
        ch);
  ...
  _az_update_json_writer_state(ref_json_writer,
                               0,
                               required_size,
                               true,
...
AZ_NODISCARD az_result az_json_writer_append_string(
  az_json_writer *ref_json_writer,
  az_span value)
{
  _az_PRECONDITION_NOT_NULL(ref_json_writer);
...
static AZ_NODISCARD az_result
az_json_writer_append_property_name_small(az_json_writer *ref_json_writer,
                                          az_span value)
{
  _az_PRECONDITION(az_span_size(
                     value) <= _az_MAX_UNESCAPED_STRING_SIZE_PER_CHUNK);
...
static AZ_NODISCARD az_result
az_json_writer_append_property_name_chunked(az_json_writer *ref_json_writer,
                                            az_span value)
{
  _az_PRECONDITION(az_span_size(value)
                   > _az_MAX_UNESCAPED_STRING_SIZE_PER_CHUNK);
...
az_json_writer_append_property_name(az_json_writer *ref_json_writer,
                                    az_span name)
{
...
AZ_NODISCARD az_result
az_json_writer_append_json_text(az_json_writer *ref_json_writer,
                                az_span json_text)
{
  _az_PRECONDITION_NOT_NULL(ref_json_writer);
  ...
  _az_RETURN_IF_FAILED(_az_validate_json(json_text, &first_token_kind,
                                         &last_token_kind));
...
AZ_NODISCARD az_result az_json_writer_append_bool(
  az_json_writer *ref_json_writer,
  bool value)
{
...
AZ_NODISCARD az_result az_json_writer_append_null(
  az_json_writer *ref_json_writer)
{
...
AZ_NODISCARD az_result az_json_writer_append_int32(
  az_json_writer *ref_json_writer,
  int32_t value)
{
  _az_PRECONDITION_NOT_NULL(ref_json_writer);
  ...
  int32_t written
    = required_size
      + _az_span_diff(leftover, remaining_json) - _az_MAX_SIZE_FOR_INT32;
  _az_update_json_writer_state(ref_json_writer,
                               written,
                               written,
                               true,
  ...
  int32_t fractional_digits)
{
  ...
  // cases.
  az_span leftover;
  _az_RETURN_IF_FAILED(az_span_dtoa(remaining_json, value, fractional_digits,
                                    &leftover));
  ...
  int32_t written
    = required_size
      + _az_span_diff(leftover,
                      remaining_json) - _az_MAX_SIZE_FOR_WRITING_DOUBLE;
  _az_update_json_writer_state(ref_json_writer,
                               written,
                               written,
                               true,
  ...
  az_json_token_kind container_kind)
{
  ...
  _az_PRECONDITION(
    ...
    || container_kind == AZ_JSON_TOKEN_BEGIN_ARRAY);
  _az_PRECONDITION(_az_is_appending_value_valid(ref_json_writer));

  // The current depth is equal to or larger than the maximum allowed depth of 64. Cannot write the
  // next JSON object or array.
  if (ref_json_writer->_internal.bit_stack._internal.current_depth
      >= _az_MAX_JSON_STACK_SIZE) {
  ...
  if (container_kind == AZ_JSON_TOKEN_BEGIN_OBJECT) {
    _az_json_stack_push(&ref_json_writer->_internal.bit_stack,
                        _az_JSON_STACK_OBJECT);
  } else {
    _az_json_stack_push(&ref_json_writer->_internal.bit_stack,
...
AZ_NODISCARD az_result az_json_writer_append_begin_object(
  az_json_writer *ref_json_writer)
{
  return _az_json_writer_append_container_start(ref_json_writer,
                                                '{',
  ...
  az_json_token_kind container_kind)
{
  ...
  _az_PRECONDITION(
    ...
    || container_kind == AZ_JSON_TOKEN_END_ARRAY);
...
AZ_NODISCARD az_result az_json_writer_append_end_object(
  az_json_writer *ref_json_writer)
{
  return az_json_writer_append_container_end(ref_json_writer,
                                             '}',
                                             AZ_JSON_TOKEN_END_OBJECT);
}

AZ_NODISCARD az_result az_json_writer_append_end_array(
  az_json_writer *ref_json_writer)
{
  return az_json_writer_append_container_end(ref_json_writer,
                                             ']',
